...
if [ -z "${1}" -o -z "${2}" -o -z "${3}" ]; then
  ...
  echo "usage: ./make_etascan_param_templates.sh <target basename> <template_template.params> '<eta values as space separated string enclosed in single quotes>'"
...
for eta in ${3}; do
  ...
  cp ${2} ${targetfile}
  sed -i "s/ETA/${texteta}/g" ${targetfile}
done
